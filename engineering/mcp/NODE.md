...
MCP is treated as an untrusted boundary — input validation at the MCP layer is stricter than internal backend route validation because MCP clients are not authenticated the same way as UI users. Every tool call is scoped to a `company_id`.
...
The first shipped MCP tool is `approval-create`, allowing external agents to programmatically request approval on any issue. This bridges MCP into Paperclip's governance layer.
