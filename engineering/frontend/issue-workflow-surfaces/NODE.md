...
Issue workflow surfaces receive live updates so that status changes, assignment changes, and other workflow transitions are reflected in real time without requiring manual refresh. This is critical for multi-agent environments where issues change status frequently as agents pick up, work on, and complete tasks.
