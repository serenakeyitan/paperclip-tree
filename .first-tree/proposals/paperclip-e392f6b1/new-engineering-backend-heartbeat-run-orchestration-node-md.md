...
target_node: new
rationale: The tree documents the heartbeat protocol (product/agent-model) and adapter interfaces but has no node for the server-side run orchestration — scheduling, lifecycle state management, failure handling, and the hardening this PR introduced.
---
