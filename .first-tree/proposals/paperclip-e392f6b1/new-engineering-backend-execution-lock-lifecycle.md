...
target_node: new
rationale: No existing tree node covers execution lock lifecycle — how stale locks from crashed or timed-out agents are detected, reclaimed, and cleaned up. The governance node covers approval gates but not execution locks; the execution workspaces proposal mentions exclusive access but not stale lock handling.
---
