...
target_node: new
rationale: PR refreshes worktree tooling and dev scripts; the tree has no node for developer tooling conventions — existing nodes cover execution workspaces and CLI commands but not the DX tooling layer (worktree management scripts, local dev workflows, tool surface decisions).
---
# Developer Tooling
