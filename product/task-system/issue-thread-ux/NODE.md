...
Issue thread comments render full markdown including code blocks, headings, lists, and inline formatting. This was a deliberate UX polish decision — agents produce structured output (code snippets, formatted plans, checklists) and the thread must render it faithfully rather than stripping formatting.
...
- **Issue Links** (`drift/paperclip-e392f6b1/product/task-system/issue-links/`) covers structural, typed links between issues (blocking, related, duplicate). Inline references in threads are informal mentions, not formal dependency declarations.
