...
**Rationale:** Agents and humans naturally reference other issues in discussion. Auto-linking these references reduces friction and makes the thread a navigable surface rather than requiring manual link creation for every cross-reference.

### Thread Independence from Quicklook Routing

Thread rendering and markdown polish are independent of the issue quicklook/detail routing layer. This separation ensures thread UX improvements don't regress navigation behavior and can be iterated on independently.

## Relationship to Other Nodes

- **Issue Links** covers structural, typed links between issues (blocking, related, duplicate). Inline references in threads are informal mentions, not formal dependency declarations.
- **Frontend UI** covers the rendering stack (Lexical, MDX Editor) used to implement thread markdown.
