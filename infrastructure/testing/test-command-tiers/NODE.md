...
The default `pnpm test` command runs the Vitest suite and nothing else. It does not run Playwright. This keeps the inner loop fast for both humans and agents and prevents unrelated browser flakiness from blocking unrelated work. `pnpm test:run` is the non-watch variant; `pnpm test:watch` is the interactive Vitest mode.
