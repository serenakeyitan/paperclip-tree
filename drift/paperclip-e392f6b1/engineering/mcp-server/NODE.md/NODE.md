...
- **approval-create** — Enables external agents to trigger governance approval workflows on issues, bridging the governance model with adapter-connected agent runtimes.
...
- **Docker-aware packaging.** The MCP server manifest is included in the Docker deps stage so the server can be built and deployed alongside the main backend.
