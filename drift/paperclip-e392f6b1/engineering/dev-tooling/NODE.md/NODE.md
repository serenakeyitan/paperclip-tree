...
**Source:** `scripts/`, CLI `worktree` commands, development documentation.

---

## Worktree Tooling

Paperclip uses git worktrees as the primary isolation mechanism for concurrent agent development. The CLI provides worktree management commands that handle creation, linking to agent sessions, and cleanup. Worktree tooling bridges the gap between the task system's logical assignment and the physical filesystem workspace where agents execute.
