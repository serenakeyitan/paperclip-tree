...
The MCP server (`engineering/mcp-server`) exposes the approval-create tool. The frontend renders approval cards on the board. The governance node's deferred list of "fine-grained per-action governance gates" is now partially addressed by this feature.
